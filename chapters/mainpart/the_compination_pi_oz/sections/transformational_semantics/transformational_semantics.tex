...
\refFig{comp_oz_pi_statefull_vm_non_atomic} shows how the $\pi$-OZ specification of $VM$ implements the non-atomic reaction concept. In the interface part it defines the needed channels. For $coffee$ four channels: one external , one internal and two for done signaling. The behavior sequence part $VM\_PI = ex\_coffee().in\_coffee<>.done\_in\_coffee() .... $ reflects exactly the numbered arrows shown in \refFig{non_atomic_reactoin}. 
