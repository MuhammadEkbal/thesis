...
\item In \refFig{fig_oz_active_idle_shop}  all the parameters of the operation schema $talk$ are input paramaters.
\end{itemize}
Why this restriction? Because a channel in \picalc{} is \findex[channel!unidirectional]{unidirectional} pro reaction. In the next chapter we will map the \oz{} class constructs to  \picalc{} constructs, so we will map an \oz{} operation to an \picalc{} name, i.e., channel. In \picalc{} a processes can send or receive over a channel pro reaction, but not the both together.