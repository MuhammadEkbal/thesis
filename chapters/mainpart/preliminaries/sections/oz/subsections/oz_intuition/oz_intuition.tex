...
\item In \oz{}: the set $coffee$ can be described using an \findex{operation schema}, which is a box named with the operation name added to the class box as shown in \refFig{fig_oz_vm_operation_schema} left.
\end{itemize}
...
\item In \oz{}: the set $talk$ can be described using an \findex{operation schema}, as shown in \refFig{oz_vm_with_operation_input_output_parameters}. We can notice that this operation doesn't change any state variable's value, it just says that the value of the output parameter $y$, written as $y!$, must be equal to the value of the state variable $message$. For input parameter use $?$ symbol.
\input{./images/preliminaries/oz/oz_vm_with_operation_input_output_parameters}
\end{itemize}

\subsubsection{\findex[instance reference]{Instance reference}:} 
