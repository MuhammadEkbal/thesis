...
\subsubsection{\findex{Guarded sum}:} Is the \findex{choice} between multiple guarded processes. If the guard of one process took place, other guarded processes will be discarded. For example, the processes: $\procchoice{\inp{x}{}.A}{\inp{b}{}.B}$ will evolve to the process $A$ if the guard $\inp{x}{}$ occurred.
