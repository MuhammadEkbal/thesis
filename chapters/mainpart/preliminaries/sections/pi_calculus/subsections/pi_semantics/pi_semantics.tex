...
The \findex[transition system!for \picalc{}]{labelled transition system} $(\procs,\traces)$ of \picalc{} processes over the action set $\actions$ has the process expressions $\procs$ as its states, and its transitions $\traces$ are those which can be inferred from the rules in \refFig{fig_transition_rules}.
...
\input{./images/preliminaries/pi_calculus/transition_rules}
\end{definition}
...
\input{./images/preliminaries/pi_calculus/inference_tree}
