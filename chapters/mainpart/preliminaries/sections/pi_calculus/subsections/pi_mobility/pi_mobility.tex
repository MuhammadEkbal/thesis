% mainfile: ../../piOZ.tex
As mentioned previously, the word \textit{mobile} refers to the information sharing and evolution, since the receiver process may use the received information to change its location. For example, consider the process: $Q\define\procres{x,y}{(\procpar{\proccall{A}{x,y}}{\proccall{B}{x}})}$ where:
\begin{itemize}
...
\refLis{pi_mobility_stargazer_code} shows the stargazer code of $Q$.  \refFig{pi_mobility_stargazer_react} shows visualization of $Q$ before and after the interaction occurrence.
\lstinputlisting[backgroundcolor=\color{white},caption={Stargazer code for the process $Q$.},captionpos=b, label={pi_mobility_stargazer_code}]{listings/pi_mobility_stargazer_code.pi}
\raggedbottom
\begin{figure}[H]%
\centering
...
\end{figure}
Intuitively,the mobility can be noticed in \refFig{pi_mobility_stargazer_react}, since $B$ changed its position in the connection topology.
