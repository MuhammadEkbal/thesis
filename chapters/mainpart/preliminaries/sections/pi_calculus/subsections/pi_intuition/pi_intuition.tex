...
To explain the \picalc{} inuitivly we will use the ion example as in \cite{milner}. Let us imagine a positive and a negative ion. When those two ions merge, we get a new construct. The merge operation is called a \findex{reaction}, since an ion acts and the other reacts. This reaction can be seen as communication between two processes. The two processes communicate to share some information.  One process is the sender and the other is the receiver. By doing the reaction both processes evoulte to some thing new. The reaction, information sharing and evolution concepts are the core of the \picalc{}. Using those concepts we can understand the title of Milner's book \findex{communicating and mobile processes: the} \picalc{} \cite{milner}. The word \findex{communicating} refers to the \findex{reaction} concept. The word \findex{mobile} refers to the \findex{information sharing and evolution} concepts, since the receiver process can use the received information to change its location as we will see in .. .
...
The full syntax of \picalc{} process is given in \refDef{def_process_syntax}. In this thesis starting from this point, when we mention the word \findex{names} we refer to the bunch of names starting with small letters used as channel and message name.
