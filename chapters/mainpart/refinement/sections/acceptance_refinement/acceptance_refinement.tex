To compare \picalc{} processes we need to define the Success-Refinement and relate it to the very strong simulation. We start by defining the \textit{success pair} of a process.
...
where: 
\[Acceptances \define \pom{\actions}\]
...
$Acc_\inA\define\{\alpha \in \inA \mid \exists \beta \in Av_\inA \wedge \sub{\alpha}=\sub{\beta} \wedge \alpha=\substitue{\vec{y}}{\vec{z}}\beta \text{ where } \\\ \ \quad  \vec{z}=\obj{\beta} \wedge \vec{y}=\obj{\alpha} \}$
...
\end{tikzpicture}
\caption{Calculating the acceptance set $Y$ of the trace $\out{}{}$}
\label{trace_set_acceptance}
\end{figure}
We can now define the success-refinement of \picalc{} processes as follow:

\begin{definition}[\index{refinement}{Successes refinement}]
\label{def_Successes_ref}
	Let $P,Q\in\procs$, then $P$ is a \findex[Successes!refinement]{successes refinement} of $Q$ iff the inverse set inclusion of traces and successes holds:
...
\subsubsection{Success-Refinement use case:}
 We will show that if $Q$ shown in \refFig{vm_and_vmHalf} very strongly simulates $P$, then $P$ refines $Q$ in success-refinement model.
let
\[P\define(\inp{a}{x}.\out{x}{a} \mid \inp{b}{y}).P\]
\[Q\define(\inp{a}{x}.\out{x}{a} \mid \inp{b}{y}).Q + \tau.Q\]
It is clear that $Q$ very strongly simulates $P$. This result implies, according to \refCor{cor_sim_acceptance_refinement}, that $P$ refines $Q$ in the success-refinement model. Thus, we need to show that $\Traces[P]\subseteq\Traces[Q] \wedge \Successes[P]\subseteq\Successes[Q]$.
 
 \begin{itemize}
\item For $\Traces[P]\subseteq\Traces[Q]$: previously we showed that it holds. 
\item For $\Successes[P]\subseteq\Successes[Q]$: let $\epsilon$ be the empty trace, then
