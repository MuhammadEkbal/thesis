...
$P\define\procres{c}{\out{a}{c}}$ and
$Q\define\procres{k}{\out{a}{k}} + \tau.Q$
Even that Q simulates P, but the traces will have different names. To avoid this, we consider the case where P and Q have the same bound names.
\begin{figure}[H]
\centering
\begin{tikzpicture}
    % draw the sets
    ...
    \node (x1) at (-1.5,0.2) {$\bout{a}{x}$};

    \node (y1) at (1.5,0.2) {$\bout{a}{k}$};
    \node (y2) at (1.5,-0.2) {$\tau$};
\end{tikzpicture}
\caption{acceptable actions}
\end{figure}

$P\define\procres{c}{\out{a}{c}}$ and
$Q\define\procres{c}{\out{a}{c}} + \tau.Q$
...
\begin{tikzpicture}
    ...
    % the points in the sets (here I just create nodes to use them later on to position
    % the circles and the arrows
    \node (x1) at (-1.5,0.2) {$\bout{a}{c}$};

    \node (y1) at (1.5,0.2) {$\bout{a}{c}$};
    \node (y2) at (1.5,-0.2) {$\tau$};
\end{tikzpicture}
\caption{acceptable actions}
\end{figure}
...
$P\define\inp{a}{c}$ and
$Q\define\inp{a}{c} + \tau.Q$
...
\begin{tikzpicture}
    ...
    % the points in the sets (here I just create nodes to use them later on to position
    % the circles and the arrows
    \node (x1) at (-1.5,0.2) {$\inpa{a}{d}$};

    \node (y1) at (1.5,0.2) {$\inpa{a}{d}$};
    \node (y2) at (1.5,-0.2) {$\tau$};
\end{tikzpicture}
\caption{acceptable actions}
\end{figure}
...
let $P\define\procres{c}{(\out{a}{c})}$, then the traces of $P$ and $[P]$ are:
...
\begin{tikzpicture}
   ...
   \draw[] (-7.5,0) ellipse [x radius = 0.9cm, y radius = 1.2cm];
    ...
    \node (m1) at (4.5,0.8) {$\bout{a}{c}$};
    \node (m2) at (4.5,0.3) {$\bout{a}{b}$};
    \node (m3) at (4.5,-0.2) {$\bout{a}{d}$};
    \node (m4) at (4.5,-0.6) {$\dot$};
    \node (m5) at (4.5,-0.8) {$\dot$};
    \node (m6) at (4.5,-1) {$\dot$};
...
let $P\define\procres{c}{(\out{a}{c}.0 + \out{a}{c}.P)}$, then the traces of $P$ and $[P]$ are:
