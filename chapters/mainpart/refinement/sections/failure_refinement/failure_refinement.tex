...
\begin{align}
    \failures[P] \define \{(t,X) \mid \exists Q\in\procs: \ec{P} \bigstep{t} \ec{Q}\wedge X\in Refusals_\alpha\}
\label{failure}
\end{align}
where: $Refusals_\alpha = \substitue{\vec{y}}{\vec{z}} Refusals$
and: $Refusals \define \pom{\actions\setminus\set{\tau}}$.
...
\label{def_failure_ref}
	Let $P,Q\in\procs$, then $\ec{P}$ is a \findex[trace!refinement]{failure refinement} of $\ec{Q}$ ($\ec{Q}\refiF \ec{P}$) iff the inverse set inclusion of traces and failure holds:
\begin{align}
