...
Notice, we are not focusing on how to calculate the $\Traces[P]$, since it has been studied in \cite{gieseking}, nor on how to calculate the refusal set $X$ of a trace $t$, since the failure-refinement is not useful as we will show later in \refCor{cor_sim_failure_refinement}. Later in \refSec{sec_acceptance-refinement} we will focus on calculating the acceptance set of a trace.
